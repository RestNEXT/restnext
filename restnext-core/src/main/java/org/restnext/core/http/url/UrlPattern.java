...
/**
 * UrlPattern leverages Regex Pattern to represent a parameterized URL. Parameters within the URL
 * are denoted by curly braces '{}' with the parameter name contained within (e.g. '{userid}'). <p>
 * Parameter names must be formed of word characters (e.g. A-Z, a-z, 0-9, '_'). </p> <p>An optional
 * format parameter following a dot ('.') may be added to the end.  While it could be named any
 * valid parameter name, RestExpress offers special handling (e.g. within the Request, etc.) if it's
 * named 'format'. </p> Note that the format specifier allows only word characters and
 * percent-encoded characters. <p> URL Pattern examples: </p> <ul> <li>/api/search.{format}</li>
 * <li>/api/search/users/{userid}.{format}</li> <li>/api/{version}/search/users/{userid}</li> </ul>
 * <p> RestExpress parses URI paths which is described in the URI Generic Syntax IETF RFC 3986
 * specifcation, section 3.3 (http://tools.ietf.org/html/rfc3986#section-3.3). RestExpress parses
 * paths into segments separated by slashes ("/"), the segments of which are composed of unreserved,
 * percent encoded, sub-delimiters, colon (":") or asperand ("@"), each of which are defined below
 * (from the spec): </p> <p>pct-encoded   = "%" HEXDIG HEXDIG</p> <p>unreserved    = ALPHA / DIGIT /
 * "-" / "." / "_" / "~"</p> <p>reserved      = gen-delims / sub-delims</p> <p>gen-delims    = / ":"
 * / "/" / "?" / "#" / "[" / "]" / "@"</p> <p>sub-delims    = / "!" / "$" / "&amp;&amp;" / "'" / "("
 * / ")" / "*" / "+" / "," / ";" / "=" / "*"</p>
 *
 * <p>In other words, RestExpress accepts path segments containing: [A-Z] [a-z] [0-9] % - . _ ~ ! $
 * &amp;&amp; ' ( ) * + , ; = : @
 *
 * <p>RestExpress also accepts square brackets ('[' and ']'), but this is deprecated and not
 * recommended.
 *
 * @author toddf
...
public class UrlPattern implements UrlMatcher {
  ...
  // Replaces the format parameter name in the URL pattern string to match the format specifier
  // in URLs. Appended to the end of the regex string when a URL pattern contains a format
  // parameter.
  ...
  /**
   * Processes the incoming URL pattern string to create a java.classpath.regex Pattern out of it
   * and parse out the parameter names, if applicable.
   */
  public void compile() {
  ...
  /**
   * Test the given URL against the underlying pattern to determine if it matches, returning a
   * boolean to reflect the outcome.
   *
   ...
   *
   * @param url an URL string with or without query string.
   * @return a UrlMatch instance reflecting the outcome of the comparison, if matched. Otherwise,
   *     null.
   */
  @Override
