...
//        });
    ...
    private void writeError(ChannelHandlerContext ctx, Response.Status status, Throwable cause) {
...
//        map.put("developerErrorMessage", ExceptionUtils.getStackTraceAsString(cause));
        ...
        String errorContent = JsonUtils.toJson(map).concat("\r\n");

        // Create error response for the request.
        FullHttpResponse response = new DefaultFullHttpResponse(HTTP_1_1, status.getNettyStatus(),
                Unpooled.copiedBuffer(errorContent, CharsetUtil.UTF_8));

        // Add some headers.
        response.headers().set(CONNECTION, CLOSE);
        response.headers().set(DATE, HttpHeaderDateFormat.get().format(new Date()));
        response.headers().set(CONTENT_TYPE, MediaType.JSON_UTF8);
